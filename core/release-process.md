...
- **`## Unreleased` promotion**: when `CHANGELOG.md` already contains a `## Unreleased` heading (the keepachangelog convention), `--apply` promotes that heading in place to `## <next> - <date>`, preserving the hand-written prose underneath. The auto-generated draft is **not** applied in this mode (the dry-run plan reports `CHANGELOG mode: promote ## Unreleased`). Otherwise, a fresh dated heading is inserted above the latest release and the auto-draft replaces the empty bullet placeholder.
